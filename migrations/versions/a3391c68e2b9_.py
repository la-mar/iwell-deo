...
def upgrade():
    ...
    op.execute("""
        create or replace function parse_time(value character varying) returns character varying
            immutable
            parallel safe
            language plpgsql
        as $$
        begin
            value := trim(substring(value from '(\d{1,2}:\d{0,2}?\s*[amAMpmPM]{0,2})'));
            value := REGEXP_REPLACE(REGEXP_REPLACE(value,'([p]$)','pm'),'([a]$)','am');
...
def downgrade():
    ...
    op.execute("drop function parse_time;")
